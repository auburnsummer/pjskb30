import { $clearData, $finalDataList, $ranking } from "./clearData.js";
...
export const STICKERS = {
    ...
    "sbuga": "/assets/stickers/sbuga.png",
...
const $stickerRecoveryMap = localStorageSignal({}, '_x_stickerURLMap');

let stage;
export async function drawImage(containerId) {
    ...
    // each card
    for (const [idx, song] of Object.entries($finalDataList.value).slice(0, 30)) {
        // draw the base card
        const gridX = idx % 3;
...
export function addSticker(stickerURL) {
    // stickers!!!
...
export function goToImageView($viewSignal, containerId) {
    if (stage) {
        stage.destroy();
    }
...
export function backToMainView($viewSignal) {
    if (stage) {
        stage.destroy();
    }
